...
{	
	...
	static inline uint64_t ImageBase = *(uint64_t*)(__readgsqword(0x60) + 0x10);

	class FMemory
	{
	public:
		static __forceinline void* InternalRealloc(void* _a1, __int64 _a2, unsigned int _a3) {
			return ((void* (*)(void*, __int64, unsigned int)) (ImageBase + 0x0346CD80))(_a1, _a2, _a3);
		}
	...
	namespace ContainerImpl
	{
		...
		public:
			inline void Set(int32 Index, bool bValue)
			{
				...
				const uint32 Mask = (1u << (Index & (NumBitsPerDWORD - 1)));
				if (bValue)
					GetData()[Index / NumBitsPerDWORD] |= Mask;
				else
					GetData()[Index / NumBitsPerDWORD] &= ~Mask;
			}
		...
		public:
			SetElement()
				: Value(), HashNextId(-1), HashIndex(0)
			{
			}
	...
	public:
		FORCEINLINE ArrayElementType& At(int i, size_t Size = sizeof(ArrayElementType)) const { return *(ArrayElementType*)(__int64(Data) + (static_cast<long long>(Size) * i)); }
		FORCEINLINE ArrayElementType* AtPtr(int i, size_t Size = sizeof(ArrayElementType)) const { return (ArrayElementType*)(__int64(Data) + (static_cast<long long>(Size) * i)); }
	...
	*/
	...
	public:
		SparseArrayElementType& Add(const SparseArrayElementType& InValue)
		{
			FElementOrFreeListLink Link{};
			*reinterpret_cast<SparseArrayElementType*>(&Link.ElementData) = InValue;

			const int32 NewIndex = Data.Add(Link);
			AllocationFlags.Set(NewIndex, true);
			return *reinterpret_cast<SparseArrayElementType*>(&Data.GetUnsafe(NewIndex).ElementData);
		}
	...
	public:
		...
		{
			return Elements.Add(SetDataType(InValue)).GetValue();
		}
	...
	public:
		...
		// Resetea el contenido del mapa al estado por defecto (equivalente a vaciarlo).
		// Nota: esta implementación no intenta liberar memoria manualmente; solo deja la estructura en estado vacío.
		inline void Reset()
		{
			Elements = {};
		}
